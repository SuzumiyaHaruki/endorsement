...
# Node-2 single-node bootstrap for the local 1-sequencer + 3-endorser lab.
# Run this on node-2.

NODE_NAME="${NODE_NAME:-node-2}"
NODE_DATA_DIR="${NODE_DATA_DIR:-/data/nitro-data/node-2}"
...
VAL_PORT="${VAL_PORT:-52000}"
ENDORSER_PORT="${ENDORSER_PORT:-9001}"
ENDORSER_ID="${ENDORSER_ID:-A}"
...
echo
echo "[*] node-2 bootstrap finished"
