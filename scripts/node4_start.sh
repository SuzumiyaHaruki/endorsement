...
# Node-4 single-node bootstrap for the local 1-sequencer + 3-endorser lab.
# Run this on node-4.

NODE_NAME="${NODE_NAME:-node-4}"
NODE_DATA_DIR="${NODE_DATA_DIR:-/data/nitro-data/node-4}"
...
VAL_PORT="${VAL_PORT:-52002}"
ENDORSER_PORT="${ENDORSER_PORT:-9003}"
ENDORSER_ID="${ENDORSER_ID:-C}"
...
echo
echo "[*] node-4 bootstrap finished"
